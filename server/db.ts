...
// Vercel's deployed filesystem is read-only. The legacy JSON database is therefore
// placed in /tmp in the serverless runtime. Local development keeps the existing data/ path.
...
export function loadDatabase(): DatabaseSchema {
  ...
  try {
    if (fs.existsSync(DB_FILE)) {
      dbInstance = JSON.parse(fs.readFileSync(DB_FILE, 'utf8')) as DatabaseSchema;
    } else {
      dbInstance = getInitialDatabase();
      fs.writeFileSync(DB_FILE, JSON.stringify(dbInstance, null, 2), 'utf8');
    }
  } catch (error) {
    console.error('Database initialization failed; using in-memory seed data.', error);
    dbInstance = getInitialDatabase();
  }
  return dbInstance;
}
...
export function getCategories() { return db().categories.filter(c => c.isActive !== false); }
export function getAllCategoriesAdmin() { return db().categories; }
export function getProducts(options: any = {}) {
  let items = db().products.filter(p => p.isActive !== false);
  if (options.categoryId) items = items.filter(p => p.categoryId === options.categoryId);
  if (options.search) { const q = String(options.search).toLowerCase(); items = items.filter(p => `${p.name} ${p.description || ''} ${p.genericName || ''}`.toLowerCase().includes(q)); }
...
export function getProductBySlugOrId(idOrSlug: string) { return db().products.find(p => p.id === idOrSlug || p.slug === idOrSlug && p.isActive !== false); }
...
export function getOrderByNumberOrId(value?: string, phone?: string) { const o: any = db().orders.find((x: any) => x.id === value || x.orderNumber === value); if (o && phone && o.phone && o.phone !== phone && o.customerPhone !== phone) return undefined; return o; }
export function createPrescription(data: any) { const p = { id: `rx-${Date.now()}`, ...data, status: 'pending' as PrescriptionStatus, createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() }; db().prescriptions.push(p as any); persist(); return p; }
...
export function getServices() { return db().services.filter(s => s.isActive !== false); }
...
export function getArticles() { return db().articles.filter(a => a.isPublished !== false); }
...
export function createContactMessage(data: any) { const m = { id: `msg-${Date.now()}`, ...data, status: 'new', createdAt: new Date().toISOString() }; db().contactMessages.push(m as any); persist(); return m; }
export function getContactMessages() { return db().contactMessages; }
export function updateContactMessageStatus(id: string, status: string) { const m: any = db().contactMessages.find(x => x.id === id); if (!m) throw new Error('Message not found'); m.status = status; persist(); return m; }
